...
def parse_args():
    ...
    parser.add_argument("--seed_json", type=str, default="No")
...
def prepare_correction_input(all_model_inputs, all_feedback_results):
    ...
    for i in range(len(all_model_inputs)):
        ...
        # make_splited_feedback = cur_feedback.split("\n")
        # feedback_list = []
        # feedback_list.append(make_splited_feedback[0])
        # for splited_feedback in make_splited_feedback[1:]:
        #     if "major" in splited_feedback:
        #         feedback_list.append(splited_feedback)
        # cur_feedback = "\n".join(feedback_list)
    ...
    for i in range(args.num_try):
        ...
        if args.do_iterate == "Yes":
            for i in range(args.iterate_num):
                ...
                if args.use_feedback == "Yes":
                    iterative_feedback_results = await generate_concurrently(
                        iterative_model_inputs, 0, None, args, True
                    )
                    # iterative_model_inputs, 0, ["\n6.", "Correct code:"], args, True

                    iterative_correction_inputs = prepare_correction_input(
                        iterative_model_inputs, iterative_feedback_results
                    )
                    all_results = await generate_concurrently(iterative_correction_inputs, 0, None, args, False)
                else:
                    all_results = await generate_concurrently(iterative_model_inputs, 0, None, args, False)
                eval_results = evaluation(all_results, args)
                ...
                # with open(
                #     os.path.join(save_path, f"{i+1}_wrongonly.json"),
                #     "w",
                #     encoding="UTF-8",
                # ) as f:
                ...
                with open(
                    os.path.join(save_path, f"{i+1}_public_wrongonly.json"),
                    "w",
                    encoding="UTF-8",
                ) as f:
