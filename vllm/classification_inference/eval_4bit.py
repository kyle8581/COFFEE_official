...
class DataArguments:
    ...
    feedback_dir: str = field(
        default=None,
        metadata={"help": "Path to the directory containing the feedback data."},
    )
...
class DataCollatorForClassification(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict:
        sources = [f"{self.tokenizer.bos_token}{example['text']}" for example in instances]
        ...
        padded_tokens = self.tokenizer.pad(tokenized_targets, padding=True, return_tensors="pt")
        data_dict = {
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    """
    make_list = []
    with open(args.feedback_dir, "r") as f:
    ...
    text_list = []
    for data_list in make_list:
        for data in data_list:
            make_text = instruction.format(
                description=data["description"], wrong_code=data["wrong_code"], feedback=data["prediction"]
            )
            text_dict = {"text": make_text}
            text_list.append(text_dict)
        none_text = instruction.format(description=data["description"], wrong_code=data["wrong_code"], feedback="None")
        text_dict = {"text": make_text}
        text_list.append(text_dict)

    make_dataset = Dataset.from_pandas(pd.DataFrame(data=text_list))
    eval_dataset = DatasetDict({"eval": make_dataset})
    # Load dataset.

    data_collator = DataCollatorForClassification(tokenizer=tokenizer, source_max_len=args.source_max_len, args=args)
    return dict(
        eval_dataset=eval_dataset["eval"] if args.do_eval else None,
        predict_dataset=eval_dataset["eval"] if args.do_predict else None,
...
def train():
    ...
    if args.do_predict:
        ...
        metrics = {"predict_" + k: v for k, v in predictions.metrics.items()}
        predict_and_label = {"prediction": predictions.predictions.tolist()}
        # trainer.log_metrics("predict", metrics)
        # trainer.save_metrics("predict", metrics)
        all_metrics.update(predict_and_label)
        all_metrics.update(metrics)
    dir_path = os.path.dirname(args.feedback_dir)
    with open(os.path.join(dir_path, "result_new.json"), "w") as f:
