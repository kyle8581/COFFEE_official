    ...
    def format_dataset(dataset, dataset_format):
        if (
            ...
            with open(os.path.join(args.output_dir, "input_output_format.json"), "w") as f:
                ...
                output_sample = dataset["train"][0]["output"]
                json.dump({"input": input_sample, "output": output_sample}, f, indent=4)
        elif dataset_format == "new_feedback_code_edit":
            instruction = "Provide feedback on the errors in the given code and suggest the correct code to address the described problem."
        ...
        elif dataset_format == "codegen-only":
            instruction = "Provide feedback on the errors in the given code and suggest the correct code to address the described problem."
        ...
        elif dataset_format == "feedback-only":
            instruction = "Provide feedback on the errors in the given code."

            dataset = dataset.map(
                lambda x: {
                    "input": f"{instruction}\nProblem Description: {x['description']}\nIncorrect Code:\n{x['wrong_code']}\n",
                    "output": f"Feedback: {x['feedback']}",
                }
            )
            if not os.path.isdir(args.output_dir):
                os.makedirs(args.output_dir, exist_ok=True)
        ...
        elif dataset_format == "octocoder_without_feedback":
            instruction = "Revise the provided code to solve the described problem correctly."

            dataset = dataset.map(
                lambda x: {
                    "input": f"Question: {instruction}\nProblem Description:{x['description']}\nIncorrect Code:\n{x['wrong_code']}\n",
                    "output": f"Answer: Correct code:\n{x['correct_code']}",
                }
            )
            if not os.path.isdir(args.output_dir):
                os.makedirs(args.output_dir, exist_ok=True)
            with open(os.path.join(args.output_dir, "input_output_format.json"), "w") as f:
                ...
                output_sample = dataset["train"][0]["output"]
                json.dump({"input": input_sample, "output": output_sample}, f, indent=4)
        elif dataset_format == "octocoder_feedback":
            instruction = "Provide feedback on the errors in the given code and suggest the correct code to address the described problem."

            dataset = dataset.map(
                lambda x: {
                    "input": f"Question: {instruction}\nProblem Description:{x['description']}\nIncorrect Code:\n{x['wrong_code']}\n",
                    "output": f"Answer: Feedback:\n{x['feedback']}\nCorrect code:\n{x['correct_code']}",
                }
            )
            if not os.path.isdir(args.output_dir):
                os.makedirs(args.output_dir, exist_ok=True)
