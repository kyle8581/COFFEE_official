...
class DataCollatorForClassification(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict:
        ...
        targets = [example["label"] for example in instances]
        labels = torch.tensor(targets)
        args = self.args
        torch_dtype = torch.float32 if args.fp16 else (torch.bfloat16 if args.bf16 else torch.float32)
        labels = labels.to(torch_dtype)
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def format_dataset(dataset, dataset_format):
        ...
        with open(os.path.join(args.output_dir, "input_output_format.json"), "w") as f:
            input_sample = dataset["train"][0]["text"]
            output_sample = dataset["train"][0]["label"]
            json.dump({"input": input_sample, "output": output_sample}, f, indent=4)

        dataset = dataset.remove_columns(
            [col for col in dataset.column_names["train"] if col not in ["text", "label"]]
        )
        return dataset
